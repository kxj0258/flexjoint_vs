...
int main(int argc, char* argv[])
{
    ...
    // Warm up camera by discarding the first few frames.
    ...
    while (g_running) {
        // Refresh the latest motor state before running this control cycle.
        ...
        // First safety layer: clamp the commanded velocity magnitude.
        float velocity = joint_cal;
        velocity = std::clamp(velocity, -kVelocitySaturation, kVelocitySaturation);

        // Second safety layer: stop motion that would push farther past angle limits.
        ...
        // Third safety layer: stop if any feature point approaches the image edge.
